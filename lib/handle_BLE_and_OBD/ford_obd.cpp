...
void notifyCallback(BLERemoteCharacteristic* pChar, uint8_t* data, size_t length, bool isNotify) {
  ...
  for (int i = 0; i < length; i++) {
    ...
    if (data[i] >= 32 && data[i] <= 126) {
      ...
      if (data[i] == 0x0D) fordOBD.response += "\r";
      else if (data[i] == 0x0A) fordOBD.response += "\n";
    }
  }
  DEBUG_PRINTLN();
...
void FordOBD::initializePIDConfig() {
  ...
  if (currentPIDIndex >= TOTAL_PIDS) currentPIDIndex = 0;
}
...
void FordOBD::fastPollingLoop() {
  ...
  switch (nb_rx_state) {
    ...
    case ELM_ERROR:
      ...
      if (now - lastSuccessfulPID > 20000) { // 20 seconds
...
void FordOBD::parseOBDData(String data) {
  ...
  for (int i = 0; i < TOTAL_PIDS; i++) {
    if (!userPIDs[i].enabled) continue;
    
    String expectedPID = String(userPIDs[i].cmd).substring(2, 4);
    if (pid == expectedPID) {
      String result = "";
      
      if (pid == "05") {  // Coolant Temperature
        if (data.length() >= 6) {
          int temp = hexToInt(data.substring(4, 6)) - 40;
          result = String(temp);
        }
      }
      else if (pid == "5C") {  // Engine Oil Temperature
        if (data.length() >= 6) {
          int temp = hexToInt(data.substring(4, 6)) - 40;
          result = String(temp);
        }
      }
      else if (pid == "0F") {  // Intake Air Temperature
        if (data.length() >= 6) {
          int temp = hexToInt(data.substring(4, 6)) - 40;
          result = String(temp);
        }
      }
      else if (pid == "0C") {  // Engine RPM
        if (data.length() >= 8) {
          int rpm = ((hexToInt(data.substring(4, 6)) << 8) + hexToInt(data.substring(6, 8))) / 4;
          result = String(rpm);
        }
      }
      else if (pid == "0D") {  // Vehicle Speed
        if (data.length() >= 6) {
          int speed = hexToInt(data.substring(4, 6));
          result = String(speed);
        }
      }
      else if (pid == "0B") {  // Intake Manifold Pressure (BOOST for EcoBoost!)
        if (data.length() >= 6) {
...
bool FordOBD::isPIDReadyToSend(int pidIndex) {
  if (pidIndex >= TOTAL_PIDS) return false;
  if (!userPIDs[pidIndex].enabled) return false;
